...
export default class CameraScreen extends React.Component {
	...
	takePicture = async () => {
		if (this.camera) {
			...
			if (data) {
				// if onPictureSaved is defined, then here data is always null
				console.log('Picture saved: ' + JSON.stringify(data.uri));
				if (data.exif) {
					console.log('Exif Data:\n' + JSON.stringify(data.exif));
					// console.log("Orientation: " + JSON.stringify(data.exif.Orientation));
					Alert.alert('Orientation: ' + data.exif.Orientation);
				} else {
	...
	renderNoPermissions = () => (
		<View style={styles.noPermissions}>
			<Text style={{ color: 'white' }}>Camera permissions not granted - cannot open camera preview. dd</Text>
		</View>
	);
